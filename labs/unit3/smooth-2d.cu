...
__global__ void smooth1D(float* values_in, float* values_out, int N)
{
    ...
    // Temporary variable to hold the weighted sum
    float tmp = 0.0f;

    // Apply the 1D smoothing using the 1D Gaussian kernel
    for (int k = -RADIUS_1D; k <= RADIUS_1D; ++k)
    {
        int neighbor_idx = min(max(idx + k, 0), N - 1);  // Clamp to boundary
        int kernel_idx = k + RADIUS_1D; // Translate kernel index to [0, 2*RADIUS_1D]
        tmp += values_in[neighbor_idx] * KERNEL_1D[kernel_idx];
    }
